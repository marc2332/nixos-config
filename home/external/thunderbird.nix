...
let
  ...
  cfg = config.programs.thunderbird-extra;

  enabledAccounts = attrValues (
    filterAttrs (_: a: a.thunderbird-extra.enable) config.accounts.email.accounts
  );
  ...
  profilesWithId = imap0 (i: v: v // { id = toString i; }) (attrValues cfg.profiles);
  ...
  toThunderbirdIdentity =
    ...
    let
      id = if address == account.address then account.id else builtins.hashString "sha256" address;
    in
    {
    ...
    // account.thunderbird-extra.perIdentitySettings id;
  ...
  toThunderbirdAccount =
    ...
    in
    {
      "mail.account.account_${id}.identities" = concatStringsSep "," (
        [ "id_${id}" ] ++ map (address: "id_${builtins.hashString "sha256" address}") account.aliases
      );
    ...
    // optionalAttrs (account.imap != null) {
      "mail.server.server_${id}.directory" = "${
        if profile.absolutePath != "" then
          ...
          "${thunderbirdProfilesPath}/${profile.name}"
      }/ImapMail/${id}";
      ...
      "mail.server.server_${id}.authMethod" = if account.aerc.imapAuth == "xoauth2" then 10 else 3;
    }
    ...
    // account.thunderbird-extra.settings id;
...
in
{
              ...
              { config, name, ... }:
              {
                options = {
                  ...
                  absolutePath = mkOption {
                    type = types.str;
                    default = "";
                    description = "This profile's absolute path.";
                  };
  ...
  config = mkIf cfg.enable {
    assertions = [
      (
        let
          defaults = catAttrs "name" (filter (a: a.isDefault) profilesWithId);
        in
        {
      ...
      (
        let
          ...
          selectedProfiles = concatMap (a: a.thunderbird-extra.profiles) enabledAccounts;
        in
        {
          assertion = (intersectLists profiles selectedProfiles) == selectedProfiles;
          message =
    ...
    ++ optional (any (p: p.withExternalGnupg) (attrValues cfg.profiles)) pkgs.gpgme;

    home.file = mkMerge (
      [
        {
          "${thunderbirdConfigPath}/profiles.ini" = mkIf (cfg.profiles != { }) {
            text = generators.toINI { } profilesIni;
          };
        }
      ]
      ++ flip mapAttrsToList cfg.profiles (
        name: profile: {
          "${
            if profile.absolutePath != "" then profile.absolutePath else "${thunderbirdProfilesPath}/${name}"
          ...
          "${
            ...
            let
              accounts = filter (
                a: a.thunderbird-extra.profiles == [ ] || any (p: p == name) a.thunderbird-extra.profiles
